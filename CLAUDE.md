...
- 2026-07-09 EVERY new `app/api/**` route MUST call `requireAuth(req)` at the top (before any key use / upstream fetch) — the password gate (`lib/auth.ts`) only protects routes that opt in; an unguarded new route is an open hole past the gate (PR #26 review; relevant to `/api/memory` in #4).
- 2026-07-09 run a worker's e2e on an ISOLATED port + own build (`reuseExistingServer:false`, unique `-p`) when another lane may hold 3010 — else Playwright reuses the OTHER branch's server and tests the wrong code; a throwaway config beside the base one does it (PRs #26/#27 close-out). NB `pnpm start` hardcodes `-p 3010` (ignores `PORT`) → use `npx next start -p <port>`.
- 2026-07-09 hydrating targets (Next.js) DELETE the injected `<base href>` and rewrite `src`/`srcset` back to root-relative on hydration — ingest-time absolutization + `<base>` both get undone. Served-HTML assertions are NOT proof of rendering: image/asset fixes must assert post-hydration `naturalWidth>0` in a REAL browser, and the fix must survive hydration (runtime-side `MutationObserver` re-absolutizing `img[src^="/"]`, or strip the site's non-runtime scripts) (PR #31 review).
...
- 2026-07-09 local `@ai` e2e suites flake hard under Playwright's default parallel workers (N concurrent live-agent flows starve one Next server → empty-page/`locator.fill` timeouts); run `@ai` specs with `--workers=1`. CI is unaffected (it's keyless + `workers:1`). External-site keyless specs (m1 "bot-wall", m2 "profile override") flake the same way locally — pass serially (PRs #22/#24).
- 2026-07-09 workers must run build/e2e in the FOREGROUND and commit BEFORE long verification — a worker that detaches a background run then ends its turn strands all work uncommitted (happened 3× on #14, twice more on #3); orchestrator recovery = TaskStop the worker, kill stray `next`/playwright PIDs holding the port, then close out (verify→commit→push→PR) from the worktree (PRs #22/#24).
