...
- 2026-07-09 run a worker's e2e on an ISOLATED port + own build (`reuseExistingServer:false`, unique `-p`) when another lane may hold 3010 — else Playwright reuses the OTHER branch's server and tests the wrong code; a throwaway config beside the base one does it (PRs #26/#27 close-out).
